...
* 非叶子节点可以拥有可变数量的子节点，当数据被插入或从一个节点中移除，它的子节点数量发生变化。为了维持在预先设定的数量范围内，内部节点可能会被合并或者分离。
* B树中每一个内部节点会包含一定数量的键，键将节点的子树分开。

#### B+树
通常被表示为一组有序的元素和子指针。
* 对于所有内部节点，子指针的数目总是与元素的数目相同。所有叶子都在相同的高度上，叶结点本身按关键字大小从小到大链接。
* 键值的拷贝被存储在内部节点；键值和记录存储在叶子节点。
...
### 预排序遍历树
一种有序的树状结构。
* 每一个节点都有一个左值和右值；每一个子节点的左值总是大于父节点的左值，右值总是小于父节点的右值，右值总是小于左值。
* 处于这些结构中的每一个节点都可以轻易的算出其祖先或后代节点。因此可以用来实现无限分类。

## 堆
给定堆中任意节点P和C，若P是C的母节点，那么P的值会小于等于（或大于等于）C的值。若母节点的值恒小于等于子节点的值，此堆称为最小堆；反之，若母节点的值恒大于等于子节点的值，此堆称为最大堆。
* 任意节点小于（或大于）它的所有后裔，最小元（或最大元）在堆的根上
* 堆总是一棵完全树。即除了最底层，其他层的节点都被元素填满，且最底层尽可能地从左到右填入。

### 二叉堆
二叉堆是完全二叉树或者是近似完全二叉树。父节点的键值总是保持固定的序关系于任何一个子节点的键值，且每个节点的左子树和右子树都是一个二叉堆。
* 二叉堆一般用数组来表示。如果根节点在数组中的位置是1，第 n 个位置的子节点分别在 2n 和 2n+1。如果存储数组的下标基于0，第 n 个位置的子节点分别在 2n+1 与 2n+2。

### 斐波那契堆
由一组最小堆有序树构成的。每个节点的度数为其子节点的数目。树的度数为其根节点的度数。
* 斐波那契堆中的树都是有根的但是无序。所有树的根节点也用一个双向循环链表链接起来。
* 每个节点x包含指向父节点的指针和指向任意一个子结点。x的所有子节点都用双向循环链表链接起来，叫做x的子链表。子链表中的每一个节点y都有指向它的左兄弟和右兄弟。
* 使用一个指针指向斐波那契堆中最小元素。

## 优先队列
优先队列中的每个元素都有各自的优先级，优先级最高的元素最先得到服务；优先级相同的元素按照其在优先队列中的顺序得到服务。优先队列往往用堆来实现。
* 基本操作：插入带优先级的元素，取出具有最高优先级的元素，查看最高优先级的元素等

### 双端优先队列
通常维护两个优先队列来实现，一个按优先级从大到小，另一个按优先级从小到大。例：最大-最小堆
* 通过把数据分成两部分（比较小的部分和比较大的部分），可避免数据重复。
